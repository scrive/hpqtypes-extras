...
data SqlUpdate (mat :: MaterializedSupported) = SqlUpdate
  { sqlUpdateWhat :: SQL
  , sqlUpdateFrom :: SQL
...
data MaterializedSupported = MaterializedSupported | MaterializedNotSupported

instance Sqlable (SqlUpdate 'MaterializedSupported) where
  toSQLCommand cmd =
